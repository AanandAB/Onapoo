...
- Static flower images: keep optimized with `py -3.12 scripts/compress-images.py` (downscales to ≤900px, re-encodes q74 progressive JPEG, only overwrites if smaller). Run after adding new images. Site pages stay `force-dynamic` (NO caching) by deliberate choice — stock/prices must be 100% live.
